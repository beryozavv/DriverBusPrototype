...
public class SerializationTests
{
    ...
    public void GuidDeserialize()
    {
        ...
        {
            Id = Guid.Parse("977c16ac-995a-4c9e-b451-b38a578059b4"),
            Type = CommandType.GetEncryptionKey,
            IsEncrypted = false,
            Parameters = JsonSerializer.Serialize(newGuid)
        };
        ...
        var deserializedGuid = JsonSerializer.Deserialize<Guid>(commandD!.Parameters, BusJsonOptions.GetOptions());
        ...
        Assert.Equal(newGuid, deserializedGuid);
        
        Assert.Equal(command.Id, deserialize1?.Id);
    }
    
    [Fact]
    public void EventsDeserialize()
    {
        var driverEventDtos = GetEvents();
        var command = new Command
        {
            Id = Guid.NewGuid(),
            Type = CommandType.SendEventsBatch,
            IsEncrypted = false,
            Parameters = JsonSerializer.Serialize(driverEventDtos)
        };
        ...
        var deserialize = JsonSerializer.Deserialize<DriverEventDto[]>(commandD!.Parameters, BusJsonOptions.GetOptions());
        ...
        Assert.Equal(driverEventDtos.Length, deserialize?.Length);
        
        Assert.NotNull(deserialize1?.Parameters);
    }
